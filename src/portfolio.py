...
def allocate_capital(
    deals: pd.DataFrame, available_equity: float, max_single_asset_pct: float = 0.40, min_dscr: float = 1.25
) -> pd.Series:
    ...
    if any(
        ...
        for column in ("equity_required", "minimum_dscr", "risk_adjusted_score")
        for value in deals[column]
    ):
        raise ValueError("portfolio inputs must be finite")
    eligible = (
        (deals["equity_required"] <= available_equity)
        & (deals["minimum_dscr"] >= min_dscr)
        & (deals["risk_adjusted_score"] > 0)
    )
...
def portfolio_allocation(
    ...
    output["constraint_flag"] = output.apply(
        lambda row: "Pass" if row["eligible"] and row["recommended_allocation"] > 0 else "Review", axis=1
    )
    output["constraint_reason"] = output.apply(
        lambda row: (
            ...
            and row["recommended_allocation"] >= available_equity * max_single_asset_pct - 1e-9
            else "Allocated"
            if row["recommended_allocation"] > 0
            else "Capital limit"
...
def portfolio_risk_view(allocation: pd.DataFrame, min_dscr: float = 1.25) -> dict[str, float]:
    ...
    if invested_capital <= 0:
        return {
            ...
            "weighted_irr": 0.0,
            "minimum_dscr": 0.0,
            "dscr_breach_exposure": 0.0,
            "negative_irr_exposure": 0.0,
    ...
    return {
        ...
        "weighted_irr": float((invested["levered_irr"] * weights).sum()),
        "minimum_dscr": float(invested["minimum_dscr"].min()),
        "dscr_breach_exposure": float(
            invested.loc[invested["minimum_dscr"] < min_dscr, "recommended_allocation"].sum() / invested_capital
        ),
        "negative_irr_exposure": float(
            invested.loc[invested["levered_irr"] < 0, "recommended_allocation"].sum() / invested_capital
        ),
...
def portfolio_snapshot(
    ...
    return {
        ...
        "weighted_irr": risk["weighted_irr"],
        "minimum_portfolio_dscr": risk["minimum_dscr"],
        "concentration_limit": float(max_single_asset_pct),
        "dscr_breach_exposure": risk["dscr_breach_exposure"],

...
def monthly_debt_schedule(
    principal: float, terms: DebtTerms, draws: list[float] | None = None, noi: list[float] | None = None
) -> pd.DataFrame:
    ...
    if len(draw_values) != terms.term_months or any(
        not math.isfinite(float(value)) or value < 0 for value in draw_values
    ):
    ...
    if noi_values is not None and (
        len(noi_values) != terms.term_months
        or any(not math.isfinite(float(value)) or value < 0 for value in noi_values)
    ):
        raise ValueError("NOI must contain one finite non-negative value per term month")
    monthly_payment = (
        principal * _payment_factor(terms.annual_rate, terms.amortization_years) / 12 if principal else 0.0
    )
    ...
    for period, draw in enumerate(draw_values, start=1):
        ...
        balance += draw
        scheduled_payment = (
            interest if period <= terms.interest_only_months else min(monthly_payment, balance + interest)
        )
        principal_paid = max(0.0, min(scheduled_payment - interest, balance))
        balance = max(0.0, balance - principal_paid)
        balloon = balance if period == terms.term_months else 0.0
        dscr = (
            ((noi_values[period - 1] / 12) / scheduled_payment)
            if noi_values is not None and scheduled_payment > 0
            else np.nan
        )
        rows.append(
            {
...
def size_debt_from_constraints(
    noi: list[float], property_value: float, max_ltv: float, min_dscr: float, terms: DebtTerms
) -> dict[str, float | str]:
    ...
    if (
        not noi_values
        or any(not math.isfinite(float(value)) or value <= 0 for value in noi_values)
        or not math.isfinite(property_value)
        or property_value <= 0
        or not 0 < max_ltv <= 1
        or min_dscr <= 0
    ):
    ...
    return {
        ...
        "dscr_limit": float(dscr_limit),
        "recommended_loan": float(recommended),
        "implied_ltv": float(recommended / property_value),
        "binding_constraint": "LTV" if ltv_limit <= dscr_limit else "DSCR",
        "annual_debt_service": float(recommended * dsf),
    }

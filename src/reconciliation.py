...
def build_reconciliation_workbook(
    ...
    for row in range(4, monthly_end + 1):
        ...
        debt_schedule.cell(row, 5, f"=-IPMT(D{row},A{row},Inputs!$B$14*12,Inputs!$B$4*Inputs!$B$12)")
        debt_schedule.cell(row, 6, f"=-PMT(D{row},Inputs!$B$14*12,Inputs!$B$4*Inputs!$B$12)")
        debt_schedule.cell(row, 7, f"=-PPMT(D{row},A{row},Inputs!$B$14*12,Inputs!$B$4*Inputs!$B$12)")

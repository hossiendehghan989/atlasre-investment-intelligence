...
def test_portfolio_allocation_caps_each_deal_and_reports_unallocated_capital():
    ...
    assert output["recommended_allocation"].tolist() == [1_000_000, 1_000_000]
    assert output["recommended_allocation"].max() <= output["equity_required"].max()
    assert output["unallocated_equity"].unique().tolist() == [8_000_000.0]
    assert set(output["constraint_reason"]) == {"Equity requirement cap"}

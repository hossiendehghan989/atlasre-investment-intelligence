...
def test_lease_inputs_reject_invalid_month_counters(bad):
    inputs = LeaseUnderwritingInputs((Lease("Tenant", date(2025, 1, 1), date(2025, 12, 31), 120_000),), date(2025, 1, 1), bad)
    with pytest.raises(ValueError):
        inputs.validate()
...
def test_debt_terms_reject_invalid_counters_or_rates(bad):
    terms = replace(DebtTerms(), annual_rate=bad) if isinstance(bad, float) and not np.isfinite(bad) else replace(DebtTerms(), term_months=bad)
    with pytest.raises(ValueError):
        terms.validate()


@pytest.mark.parametrize("field", ["construction_months", "stabilization_months", "hold_months_after_stabilization", "noi_ramp_months"])
...
def test_public_numeric_boundaries_reject_bad_values(bad):
    deals = pd.DataFrame([{"asset": "A", "equity_required": 100, "risk_adjusted_score": 1, "levered_irr": .1, "minimum_dscr": 1.5}])
    with pytest.raises(ValueError):
        portfolio_allocation(deals, 1_000, min_dscr=bad)
    with pytest.raises(ValueError):
        size_debt(100, .06, .6, bad, .07, 25, 1_000)
    with pytest.raises(ValueError):
